...
export interface IAlertingService {
   ...
   listAlerts(user: UserInsideAuthorizedRequest, query: ListAlertsQueryDTOType): Promise<{ data: AlertingDocument[]; nextCursor?: string }>;
   getAlert(user: UserInsideAuthorizedRequest, clientId: string, alertId: string): Promise<AlertingDocument>;
   updateAlert(user: UserInsideAuthorizedRequest, clientId: string, alertId: string, data: UpdateAlertDTOType): Promise<AlertingDocument>;
   deleteAlert(user: UserInsideAuthorizedRequest, clientId: string, alertId: string): Promise<void>;
   setEnabled(user: UserInsideAuthorizedRequest, clientId: string, alertId: string, isEnabled: boolean): Promise<AlertingDocument>;
   getAlertHistory(user: UserInsideAuthorizedRequest, clientId: string, alertId: string, query: AlertHistoryQueryDTOType): Promise<{ data: AlertFireLogDocument[]; nextCursor?: string }>;
   getIncidents(user: UserInsideAuthorizedRequest, clientId: string, query: AlertHistoryQueryDTOType): Promise<{ data: IncidentFeedItem[]; nextCursor?: string }>;
}
